...
def ngons(args):
    ...
    if kk == depth and splits == 3:
        ...
        geostraight = ghc.Extrude(pgn, vect) #extrudes the polygon along vector
        geo = ghc.Taper(geostraight, axis, radius, radiusend, False, False, False) #inputs: geometry, axis, start radius, end radius, flat (False), infinite (False), rigid (False)
