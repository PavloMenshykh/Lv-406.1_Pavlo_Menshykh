...
def fractal(depth, x2, y2, z2, x3, y3, z3, length, anglerec, angle, lvariation, aran, lran, anglerech, angleh):
    ...
    if depth:
        ...
        rotplane3 = ghc.PlaneNormal(vecend, movevec)[0]
        ...
        z2 = z3
        x3 = x3
        leny = (length+lrn)*sin(radians(anglerec))
        lenz = (length+lrn)*cos(radians(anglerec))
        ...
        endpoint1 = ghc.Move(stpoint, ampz)[0]
        endpoint = ghc.Move(endpoint1, ampy)[0]
        ...
        x2 = rg.Point3d(vecend)[0]
        y2 = rg.Point3d(vecend)[1]
        z2 = rg.Point3d(vecend)[2]
        ...
        for aa in ahr:
            fractal(depth - 1 , x2, y2, z2, x3, y3, z3, length, angle, angle, lvariation, aran, lran, aa, angleh)
